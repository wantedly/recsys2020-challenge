    ...
    def _read_features_from_bigquery(
        ...
        query = """
                with multilingual_features as (
                  select
                    *,
                    case when n_first_lan = n_second_lan then 1 else 0 end first_eq_second,
                    case when n_second_lan = n_third_lan then 1 else 0 end second_eq_third,
                  from
                  (
                    select
                        engaged_user_id,
