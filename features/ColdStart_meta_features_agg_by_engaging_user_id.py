...
class ColdStartMetaFeaturesAggByEngagingUserId(BaseFeature):
    ...
    def make_features(self, df_train_input, df_test_input):
        ...
        meta_features_train = self._download_from_gs(
            feather_file_name="ColdStartMetaFeatures_training.ftr"
        )
        if self.TESTING:
            meta_features_test = self._download_from_gs(
                feather_file_name="ColdStartMetaFeatures_test.ftr"
            )
        else:
            meta_features_test = self._download_from_gs(
                feather_file_name="ColdStartMetaFeatures_val_20200418.ftr"
            )
        ...
        for target_col in target_columns:
            df_train_input[target_col] = meta_features_train[f"ColdStartMetaFeatures_{target_col}"].values
            df_test_input[target_col] = meta_features_test[f"ColdStartMetaFeatures_{target_col}"].values
...
if __name__ == "__main__":
    ColdStartMetaFeaturesAggByEngagingUserId.main()
