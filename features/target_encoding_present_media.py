...
class TargetEncodingPresentMedia(BaseFeature):
        ...
        query = """
                WITH subset AS (
                    SELECT
                        tweet_id,
                        engaging_user_id,
                        any_value(present_media) AS present_medias,
                ...
                FROM subset,
                unnest(present_medias) AS present_media
                )
                , use_combination AS (
                SELECT present_media, engaging_user_id, COUNT(*) AS cnt
                FROM unnest_subset
                GROUP BY present_media, engaging_user_id
                HAVING COUNT(*) >= 1
                )
                SELECT
                    unnest_subset.tweet_id,
                    unnest_subset.engaging_user_id,
                    unnest_subset.present_media,
                ...
                INNER JOIN
                    use_combination
                    ON unnest_subset.present_media = use_combination.present_media
    ...
    def make_features(self, df_train_input, df_test_input):
        ...
        train_data = self._read_inter_table_from_bigquery(train_table)
        test_data = self._read_inter_table_from_bigquery(test_table)

        train_data["engaging_user_id__present_media"] = train_data["engaging_user_id"] + "_" + train_data["present_media"]
        test_data["engaging_user_id__present_media"] = test_data["engaging_user_id"] + "_" + test_data["present_media"]
        ...
        category_column = "engaging_user_id__present_media"
...
if __name__ == "__main__":
    TargetEncodingPresentMedia.main()
